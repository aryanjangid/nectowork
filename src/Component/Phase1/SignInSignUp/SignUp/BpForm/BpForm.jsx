...
function BpForm({onSignUpClose}) {
    ...
    const [values, setValues] = useState({
        First_Name: "", Last_Name: "", Buisness_Email_ID: "", Phone_Number: "", Password: "", Confirm_Password: ""
    });
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (values.First_Name && values.Last_Name && values.Buisness_Email_ID && values.Password && values.Confirm_Password && values.Phone_Number) {
            await axios.post(`http://127.0.0.1:8000/CreateLogin`, {
    ...
    return (<Fragment>
        <form method="post" className="bp-signup-form">
            ...
            <label className="sp-signup-label" htmlFor="submit">
                <button type="submit" className="button-signup" onClick={handleSubmit}>
                    Sign Up
                </button>
            </label>
        </form>
